    ...
    <Container>
      ...
      <PagerView style={{flex: 1}} onPageSelected={() => setIsShowAnswer(false)}>
        {lesson && lesson[type].map((item, i, arr) => (
          <View key={item.id}>
            <Text style={{marginTop: "auto", fontSize: 30, textAlign: "center", marginBottom: 50}}>{item.question}</Text>
            {!isShowAnswer && <Btn onPress={() => setIsShowAnswer(true)}>Показать ответ</Btn>}
            {isShowAnswer && <Text style={{fontSize: 30, textAlign: "center"}}>{item.answer}</Text>}
